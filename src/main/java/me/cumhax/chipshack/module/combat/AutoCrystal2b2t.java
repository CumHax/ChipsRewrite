...
public class AutoCrystal2b2t extends Module
{
	...
	private final Setting placeRange = new Setting("PlaceRange", this, 1, 5, 10);
	private final Setting attackRange = new Setting("AttackRange", this, 1, 4, 10);
	private final Setting minDamage = new Setting("MinDamage", this, 0, 4, 16);
	private final Setting enemyRange = new Setting("EnemyRange", this, 1, 9, 20);
	...
	public void onTick(TickEvent.ClientTickEvent event)
	{
		...
		if (crystal != null && mc.player.getDistance(crystal) <= (float) (Integer) attackRange.getIntValue())
		{
			if (System.nanoTime() / 1000000L - breakSystemTime >= (long) (420 - attackSpeed.getIntValue() * 20))
			{
				if (!onlyOwn.getBoolValue() || ownCrystals.contains(render))
				{
			...
			if (multiPlace.getBoolValue())
			{
				if (System.nanoTime() / 1000000L - multiPlaceSystemTime >= (long) (20 * (placeSpeed.getIntValue())) && System.nanoTime() / 1000000L - antiStuckSystemTime <= (long) (400 + (400 - attackSpeed.getIntValue() * 20)))
				{
					multiPlaceSystemTime = System.nanoTime() / 1000000L;
					return;
				}
			}
			else if (System.nanoTime() / 1000000L - antiStuckSystemTime <= (long) (400 + (400 - attackSpeed.getIntValue() * 20)))
			{
				return;
			}
		}
		...
		ArrayList<Entity> entities = mc.world.playerEntities.stream().filter((entityPlayer) -> !Client.INSTANCE.friendManager.isFriend(entityPlayer.getName())).collect(Collectors.toCollection(ArrayList::new));
		double damage = 0.5D;
		...
		while (true)
		{
				...
				do
				{
					do
					{
						if (!var11.hasNext())
						{
							...
							if (System.nanoTime() / 1000000L - placeSystemTime >= (long) (placeSpeed.getIntValue() * 5))
							{
			...
			while (mc.player.getDistanceSq(entity2) > (double) ((Integer) enemyRange.getIntValue() * (Integer) enemyRange.getIntValue()));
			...
			while (true)
			{
				...
				do
				{
					do
					{
						...
						while (d < (double) (Integer) minDamage.getIntValue() && ((EntityLivingBase) entity2).getHealth() + ((EntityLivingBase) entity2).getAbsorptionAmount() > (float) (Integer) facePlaceHealth.getIntValue());
					}
					while (d <= damage);
	...
	public void onWorldRender(RenderWorldLastEvent event)
	{
		...
		if (render != null)
		{
			...
			{
				RenderUtil.drawBoxFromBlockpos(render, r / 255f, g / 255f, b / 255f, alpha.getIntValue() / 255f);
			}
			else
			{
				RenderUtil.drawBoxFromBlockpos(render, red.getIntValue() / 255f, green.getIntValue() / 255f, blue.getIntValue() / 255f, alpha.getIntValue() / 255f);
			}
		}
	...
	private List<BlockPos> findCrystalBlocks()
	{
		...
		positions.addAll(getSphere(getPlayerPos(), ((Integer) placeRange.getIntValue()).floatValue()).stream().filter(this::canPlaceCrystal).collect(Collectors.toList()));
		return positions;
	}
